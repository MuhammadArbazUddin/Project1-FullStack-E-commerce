...
export default function Navbar() {
  ...
  // Added token, setToken, and navigate from Context
  ...
  const logout = () => {
    ...
    setToken('');
    setCartItems({}); // Clears the cart locally on logout
  };
    ...
    <div className="flex items-center justify-between py-5 font-medium">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="group relative">
          <img
            onClick={() => token ? null : navigate('/login')} // Redirect to login if no token
            className="w-5 cursor-pointer"
            src={assets.profile_icon}
            alt="profile"
          />
          
          {/* Dropdown only shows if token exists */}
          {token && (
